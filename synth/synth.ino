  ...
  for (int i = 0; i < OSCS; i++)
  {
    if (activeCh[i] == true)
    {
      ...
      switch (params[i].state)
      {
        ...
        //Attack currentLevelを増やしていって、envelopeLevelAまで大きくなったらDecayに移行
        case STATE_ATTACK:
        {
        ...
        //Decay currentLevelを減らしていって、envelopeLevelSまで小さくなったらSustainに移行
        case STATE_DECAY:
        {
    ...
    if (seqCounter >= SEQ_LENGTH)
    {
      ...
      if ((barCounter & 3) == 0)
      {
        ...
        for (int i = 0; i < CHANNELS; i++)
        {
          ...
          {
            osc = i * OPS + j;
            params[osc].envelopeDiffA = CONST03 >> serial_a;
            params[osc].envelopeDiffD = -CONST03 >> 13;
